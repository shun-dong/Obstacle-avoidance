...
class CarController:
    ...
    def calculate_avoidance_force(self):
        ...
        for point in self.scan_points:
            ...
            if distance < self.recovery_distance:
                direction = np.array([dx, dy]) / distance
                force_mag = self.obstacle_force_gain * (1 - distance/self.recovery_distance)**2
                avoidance_force -= force_mag * direction

        # 更新避障状态
        avoidance_mag = np.linalg.norm(avoidance_force)
        if avoidance_mag > 0.1:
            if not self.avoidance_active:
                self.pre_avoidance_velocity = self.velocity.copy()
    ...
    def update_movement(self, dt):
        ...
        # 避障加速度
        avoidance_acc = self.calculate_avoidance_force()

        # 恢复加速度
        recovery_acc = np.array([0.0, 0.0])
        recovery_gain = 0.5
        if not self.avoidance_active and np.linalg.norm(self.pre_avoidance_velocity) > 0:
            recovery_acc = (self.pre_avoidance_velocity - self.velocity) * recovery_gain

        # 综合加速度
        total_acc = control_acc + avoidance_acc + recovery_acc
